## This module connects to the PostgreSQL database and retrieves analysis results for the GradCafe applicants data.
## For all intents and purposes, it is the same thing as query_data.py but with a little extra functionality
## to return the results in a structured format for use in the Flask application. 
...
def get_analysis_results():
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute("SELECT COUNT(*) FROM applicants WHERE term = 'Fall 2025';")
        results.append({
            "question": "How many applicants do you have in your database table who have applied for Fall 2025?",
            "answer": f"Applicant Count (Fall 2025): {cursor.fetchone()[0]}"
        })
        ...
        """)
        results.append({
            "question": "What percentage of applicants are from international students (not American or Other)?",
            "answer": f"Percent International Applicants: {cursor.fetchone()[0]}%"
        })

        ## 3. Average GPA, GRE, GRE_V, GRE_AW analyzed separately with bounds where applicable
        cursor.execute("SELECT ROUND(AVG(gpa)::numeric, 2) FROM applicants WHERE gpa IS NOT NULL;")
        gpa = cursor.fetchone()[0]

        cursor.execute("SELECT ROUND(AVG(gre)::numeric, 1) FROM applicants WHERE gre IS NOT NULL AND gre <= 340;")
        gre = cursor.fetchone()[0]

        cursor.execute("SELECT ROUND(AVG(gre_v)::numeric, 1) FROM applicants WHERE gre_v IS NOT NULL AND gre_v <= 170;")
        gre_v = cursor.fetchone()[0]

        cursor.execute("SELECT ROUND(AVG(gre_aw)::numeric, 2) FROM applicants WHERE gre_aw IS NOT NULL AND gre_aw <= 6.0;")
        gre_aw = cursor.fetchone()[0]

        results.append({
            "question": "What is the average GPA, GRE, GRE V, GRE AW of applicants who provided these metrics?",
            "answer": f"Average GPA: {gpa}, Average GRE: {gre}, Average GRE_V: {gre_v}, Average GRE_AW: {gre_aw}"
        })
        ...
        """)
        results.append({
            "question": "What is the average GPA of applicants who applied for Fall 2025 and were accepted?",
            "answer": f"Average GPA (Accepted, Fall 2025): {cursor.fetchone()[0]}"
        })
        ...
        results.append({
            ...
            "answer": f"JHU Masters Computer Science Applicants Count: {cursor.fetchone()[0]}"
        })
